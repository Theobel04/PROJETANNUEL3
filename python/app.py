...
def load_trained_model():
    ...
    if os.path.exists(model_file):
        ...
        print(f"Fichier modèle non trouvé: {model_file}")
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .upload-area:hover { background: #f7f9fc; border-color: #764ba2; }
        .upload-icon { font-size: 48px; margin-bottom: 10px; }
        input[type="file"] { display: none; }
        .preview-container { margin: 20px 0; display: none; }
        .preview-image { max-width: 300px; border-radius: 15px; box-shadow: 0 5px 15px rgba(0,0,0,0.2); }
        .result { margin: 20px 0; padding: 20px; border-radius: 15px; display: none; }
        .result.success { background: #d4edda; color: #155724; display: block; }
        .result.error { background: #f8d7da; color: #721c24; display: block; }
        .class-name { font-size: 24px; font-weight: bold; margin: 10px 0; }
...
def health():
    return jsonify({
        ...
        'model_type': 'PMC entraîné - 1024x64x3'
    })
...
if __name__ == '__main__':
    ...
    if model is None:
        print("\nAucun modèle trouvé !")
        print("python train_model.py doit être lancé")
    else:
